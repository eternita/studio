...
public class TreeModelContentProvider implements ITreeModelContentProvider, IContentProvider, IModelChangedListener {
    ...
    FlowJavaThreadContentProvider flowStackProvider = new FlowJavaThreadContentProvider();
    BreakpointManagerContentProvider breakpointManagerContentProvider = new BreakpointManagerContentProvider();
    ...
    public BreakpointManagerContentProvider getBreakpointManagerContentProvider() {
        return breakpointManagerContentProvider;
    }
     ...
     * 
     * @param input
     *        the input to install the model proxy on
     * @param path
     *        the {@link TreePath} to install the proxy for
     */
    private void installModelProxy(Object input, TreePath path) {
        ...
        if (!fTreeModelProxies.containsKey(path) && !fModelProxies.containsKey(path.getLastSegment())) {
            ...
            if (element instanceof Launch)
            {
                ...
                if (l.getDebugTarget() instanceof FlowDebugTarget) {

                } else if (l.getDebugTarget() == null) {
     ...
     * 
     * @param path
     *        the {@link TreePath} to dispose the model proxy for
     */
     ...
     * @return
     */
    boolean isContinue(IModelDelta delta)
    {
        delta.getChildDeltas();
        Object obj = delta.getElement();
        if (obj instanceof LaunchManager)
        {
            IModelDelta[] deltas = delta.getChildDeltas();
            if (deltas != null && deltas.length == 1)
            {
                IModelDelta lDelta = deltas[0];
                if (lDelta.getElement() instanceof Launch && (lDelta.getFlags() & IModelDelta.ADDED) == 1)
                {
                    Launch launch = (Launch) lDelta.getElement();
                    if (launch.getDebugTarget() == null)
                    {
                        return true;
                    }
                    if (!(launch.getDebugTarget() instanceof FlowDebugTarget))
                    {
                        return false;
                    }
                }
     ...
     * 
     * @param viewerUpdateMonitor
     *        the update to notify about
     */
    void updateStarted(org.neuro4j.studio.debug.ui.views.ViewerUpdateMonitor viewerUpdateMonitor) {
        Assert.isTrue(getViewer().getDisplay().getThread() == Thread.currentThread());

        List requests = (List) fRequestsInProgress.get(viewerUpdateMonitor.getSchedulingPath());
        if (requests == null) {
            requests = new ArrayList();
            fRequestsInProgress.put(viewerUpdateMonitor.getSchedulingPath(), requests);
        }
        requests.add(viewerUpdateMonitor);
        ...
        if (DebugUIPlugin.DEBUG_UPDATE_SEQUENCE && DebugUIPlugin.DEBUG_TEST_PRESENTATION_ID(getPresentationContext())) {
            DebugUIPlugin.trace("\tBEGIN - " + viewerUpdateMonitor); //$NON-NLS-1$
        }
        notifyUpdate(UPDATE_BEGINS, viewerUpdateMonitor);
    }
     ...
     * 
     * @param type
     *        Type of update to call listeners with.
     * @param update
     *        Update to notify about.
     */
     ...
     * 
     * @param requests
     *        List of waiting requests to coalesce with
     * @param toCoalesce
     *        request to coalesce
     ...
     * completed.
     * <p>
     * Requests are processed in order such that updates to children are delayed until updates for parent elements are
     * completed. This allows the expansion/selection state of the elements to be properly restored as new elements are
     * retrieved from model.
     * </p>
     * 
     * @param schedulingPath
     *        schedulingPath path or requests to start processing. May
    ...
    private void doUpdateChildCount(TreePath path) {
        ...
        IElementContentProvider contentAdapter = null;
        if (element instanceof JDIThread)
        {
            contentAdapter = flowStackProvider;
    ...
    void doUpdateElement(TreePath parentPath, int modelIndex) {
        ...
        IElementContentProvider contentAdapter = null;
        if (parent instanceof JDIThread)
        {
            contentAdapter = flowStackProvider;
        } else if (parent instanceof DefaultBreakpointsViewInput)
        {
            contentAdapter = breakpointManagerContentProvider;
        } else {
            contentAdapter = ViewerAdapterService.getContentProvider(parent);
        }
     ...
     * 
     * @see
     * org.eclipse.debug.internal.ui.viewers.model.provisional.viewers.ModelContentProvider#handleContent(org.eclipse
     * .debug.internal.ui.viewers.provisional.IModelDelta)
     */
     ...
     * 
     * @see
     * org.eclipse.debug.internal.ui.viewers.model.ModelContentProvider#handleCollapse(org.eclipse.debug.internal.ui
     * .viewers.model.provisional.IModelDelta)
     */
     ...
     * 
     * @see
     * org.eclipse.debug.internal.ui.viewers.model.provisional.viewers.ModelContentProvider#handleExpand(org.eclipse
     * .debug.internal.ui.viewers.provisional.IModelDelta)
     */
     ...
     * 
     * @param delta
     *        Delta that points to the element to expand.
     */
     ...
     * 
     * @param parentPath
     *        viewer tree path to parent element
     * @param element
     *        element to insert
     * @param modelIndex
     *        index of the element in the model
    ...
    protected void handleRemove(IModelDelta delta) {
        ...
        if (modelIndex >= 0) {
            ...
            if (element instanceof FlowLineBreakpointAdapter)
            {
     ...
     * 
     * @param delta
     *        Delta pointing to the element to reveal.
     */
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.jface.viewers.ILazyTreePathContentProvider#updateChildCount(org.eclipse.jface.viewers.TreePath,
     * int)
     */
     ...
     *        Update to perform.
     */
    void scheduleViewerUpdate(org.neuro4j.studio.debug.ui.views.ViewerUpdateMonitor viewerUpdateMonitor) {
        Display display;
        ...
        synchronized (this) {
            ...
            fCompletedUpdates.add(viewerUpdateMonitor);
