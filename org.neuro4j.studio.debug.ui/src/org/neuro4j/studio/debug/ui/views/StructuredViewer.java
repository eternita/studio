 ...
 * <p>
 * Any number of viewer filters can be added to this viewer (using <code>addFilter</code>). When the viewer receives an
 * update, it asks each of its filters if it is out of date, and refilters elements as required.
 * </p>
 * 
 * @see ViewerFilter
...
public abstract class StructuredViewer extends ContentViewer implements IPostSelectionProvider {
    ...
    /**
     * A map from the viewer's model elements to SWT widgets. (key type: <code>Object</code>, value type:
     * <code>Widget</code>, or <code>Widget[]</code>). <code>null</code> means that the element map is disabled.
     */
    ...
    /**
     * This viewer's filters (element type: <code>ViewerFilter</code>). <code>null</code> means there are no filters.
     */
    ...
    protected class ColorAndFontCollectorWithProviders extends ColorAndFontCollector {
         ...
         * 
         * @see
         * org.eclipse.jface.viewers.StructuredViewer.ColorAndFontManager#applyFontsAndColors(org.eclipse.swt.widgets
         * .TreeItem)
         */
         ...
         * 
         * @see
         * org.eclipse.jface.viewers.StructuredViewer.ColorAndFontManager#applyFontsAndColors(org.eclipse.swt.custom
         * .TableTreeItem)
         */
     ...
     *        element
     * @param fullMap
     *        <code>true</code> if mappings are added and removed, and <code>false</code> if only the new map gets
     *        installed
     */
     ...
     * <p>
     * The default implementation of this method tries first to find the widget for the given element assuming that it
     * is the viewer's input; this is done by calling <code>doFindInputItem</code>. If it is not found there, it is
     * looked up in the internal element map provided that this feature has been enabled. If the element map is
     * disabled, the widget is found via <code>doFindInputItem</code>.
     * </p>
     * 
     * @param element
     *        the element
     ...
     * <p>
     * This method was introduced to support multiple equal elements in a viewer (@see {@link AbstractTreeViewer}).
     * Multiple equal elements are only supported if the element map is enabled by calling
     * {@link #setUseHashlookup(boolean)} and passing <code>true</code>.
     * </p>
     * <p>
     * The default implementation of this method tries first to find the widget for the given element assuming that it
     * is the viewer's input; this is done by calling <code>doFindInputItem</code>. If it is not found there, the
     * widgets are looked up in the internal element map provided that this feature has been enabled. If the element map
     * is disabled, the widget is found via <code>doFindItem</code>.
     * </p>
     * 
     * @param element
     *        the element
    ...
    /**
     * Returns the comparer to use for comparing elements, or <code>null</code> if none has been set. If specified,
    ...
    /**
     * Returns the item at the given display-relative coordinates, or <code>null</code> if there is no item at that
     * location or
     * the underlying SWT-Control is not made up of {@link Item} (e.g {@link ListViewer})
     * <p>
     ...
     * <p>
     * Subclasses do not typically override this method, but implement <code>getSelectionFromWidget(List)</code>
     * instead.
     * <p>
     * 
     * @return ISelection
     ...
     * one. If this viewer has a comparator that was set via <code>setComparator(ViewerComparator)</code> then this
     * method will return <code>null</code> if the comparator is not an instance of ViewerSorter.
     * <p>
     ...
     * <p>
     * This framework method is called if a model change picked up by a viewer results in an invalid selection. For
     * instance if an element contained in the selection has been removed from the viewer, the viewer is free to either
     * remove the element from the selection or to pick another element as its new selection. The default implementation
     * of this method calls <code>updateSelection</code>. Subclasses may override it to implement a different strategy
    ...
    /**
     * The <code>StructuredViewer</code> implementation of this <code>ContentViewer</code> method calls
     * <code>update</code> if the
     ...
     * <p>
     * The default implementation simply calls <code>internalRefresh(element)</code>, ignoring <code>updateLabels</code>.
     * <p>
     * If this method is overridden to do the actual refresh, then <code>internalRefresh(Object element)</code> should
     * simply call <code>internalRefresh(element, true)</code>.
     * 
     * @param element
     *        the element
     * @param updateLabels
     *        <code>true</code> to update labels for existing elements, <code>false</code> to only update labels as
     *        needed, assuming
     ...
     * <li>attempts to restore the old selection (using <code>setSelectionToWidget</code></li>
     ...
     * <li>attempts to restore the old selection (using <code>setSelectionToWidget</code></li>
     * <li>rediscovers the resulting selection (via <code>getSelection</code>)</li>
     * <li>calls <code>handleInvalidSelection</code> if the selection did not take</li>
     * </ul>
     * </p>
     * 
     * @param updateCode
     *        the code to run
     * @param reveal
     *        <code>true</code> if the selection should be made visible, <code>false</code> if scrolling should be
     *        avoided
     * @since 3.3
     */
     ...
     * viewer's model. If <code>updateLabels</code> is <code>true</code> then labels for otherwise unaffected elements
     * are updated as well.
     ...
     * <p>
     * Note that the implementation may still obtain labels for existing elements even if <code>updateLabels</code> is
     * false. The intent is simply to allow optimization where possible.
     * 
     * @param updateLabels
     *        <code>true</code> to update labels for existing elements, <code>false</code> to only update labels as
     *        needed, assuming
     ...
     * <p>
     * Unlike the <code>update</code> methods, this handles structural changes to the given element (e.g. addition or
     * removal of children). If only the given element needs updating, it is more efficient to use the
    ...
    /**
     * 
     * Refreshes the given item with the given element. Calls <code>doUpdateItem(..., false)</code>.
     * <p>
     * This method is internal to the framework; subclassers should not call this method.
     * </p>
     * 
    ...
    public void setSelection(ISelection selection, boolean reveal) {
        /**
         * <p>
         * If the new selection differs from the current selection the hook <code>updateSelection</code> is called.
         * </p>
         * <p>
         * If <code>setSelection</code> is called from within <code>preserveSelection</code>, the call to
         * <code>updateSelection</code> is delayed until the end of <code>preserveSelection</code>.
         * </p>
         * <p>
    ...
    /**
     * Converts the selection to a <code>List</code> and calls <code>setSelectionToWidget(List, boolean)</code>. The
     * selection is
     * expected to be an <code>IStructuredSelection</code> of elements. If
     * not, the selection is cleared.
     * <p>
     * Subclasses do not typically override this method, but implement <code>setSelectionToWidget(List, boolean)</code>
     * instead.
     * 
     * @param selection
     ...
     * 
     * @param enable
     *        <code>true</code> to enable hash lookup, and <code>false</code> to disable it
     */
    ...
    /**
     * Sets the comparer to use for comparing elements, or <code>null</code> to use the default <code>equals</code> and
     ...
     * disable the selection behavior may not be possible for all subclasses of <code>StructuredViewer</code>, or may
     * cause program errors. This method
     * is supported for {@link TableViewer}, {@link TreeViewer}, {@link ListViewer}, {@link CheckboxTableViewer},
     ...
     * 
     * @param preserve
     *        <code>true</code> if selection should be preserved, <code>false</code> otherwise
     */
     ...
     * update operations. To be used by subclasses that override {@link #preservingSelection(Runnable)}.
     * 
     * @return <code>true</code> if selection should be preserved, <code>false</code> otherwise
     */
     ...
     * <p>
     * This does not handle structural changes (e.g. addition or removal of elements), and does not update any other
     * related elements (e.g. child elements). To handle structural changes, use the <code>refresh</code> methods
     * instead.
     * </p>
     * <p>
     * This should be called when an element has changed in the model, in order to have the viewer accurately reflect
     * the model. This method only affects the viewer, not the model.
     * </p>
     * <p>
     * Specifying which properties are affected may allow the viewer to optimize the update. For example, if the label
     * provider is not affected by changes to any of these properties, an update may not actually be required.
     * Specifying <code>properties</code> as <code>null</code> forces a full update of the given elements.
     * </p>
     * <p>
     * If the viewer has a sorter which is affected by a change to one of the properties, the elements' positions are
     * updated to maintain the sort order. Note that resorting may not happen if <code>properties</code> is
     * <code>null</code>.
     * </p>
     * <p>
     * If the viewer has a filter which is affected by a change to one of the properties, elements may appear or
     * disappear if the change affects whether or not they are filtered out. Note that resorting may not happen if
     * <code>properties</code> is <code>null</code>.
     * </p>
     * 
     * @param elements
     *        the elements
     ...
     * Specifying which properties are affected may allow the viewer to optimize the update. For example, if the label
     * provider is not affected by changes to any of these properties, an update may not actually be required.
     * Specifying <code>properties</code> as <code>null</code> forces a full update of the element.
     * </p>
     * <p>
     * If the viewer has a sorter which is affected by a change to one of the properties, the element's position is
     * updated to maintain the sort order. Note that resorting may not happen if <code>properties</code> is
     * <code>null</code>.
     * </p>
     * <p>
     * If the viewer has a filter which is affected by a change to one of the properties, the element may appear or
     * disappear if the change affects whether or not the element is filtered out. Note that filtering may not happen if
     * <code>properties</code> is <code>null</code>.
     * </p>
     * 
     * @param element
     ...
     * <code>doUpdateItem(widget, element, true)</code>.
     * </p>
     * 
     * @param widget
     *        the widget
     ...
     * </p>
     * <p>
     * The default implementation of this method notifies all selection change listeners recorded in an internal state
     * variable. Overriding this method is generally not required; however, if overriding in a subclass,
    ...
    protected void handleDispose(DisposeEvent event) {
        ...
        if (filters != null)
            filters.clear();
        elementMap = new CustomHashtable(1);
        openListeners.clear();
