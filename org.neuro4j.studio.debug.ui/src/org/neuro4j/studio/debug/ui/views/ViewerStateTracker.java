 ...
 * viewer.
 * <p>
 * When the input to the viewer is changes, the tree model viewer attempts to save the expansion state of elements as
 * well as currently selected element and scroll position. Each expanded element is queried for its memento and all the
 * collected mementos are saved into a delta tree then serialized by the viewer.<br>
 * When a new input is set to the viewer, the viewer compares the input's memento with the stored mementos and if a
 * match is found, it attempts to restore the previous expansion state to the viewer. As elements are revealed and
 * realized in the viewer, the element's memento is compared against the memento stored in the saved state delta. If
 * matching elements are found in the delta, the expansion and selection state is then restored to those elements.
 * </p>
 * <p>
 * Additionally to saving restoring state on input change, the viewer also saves/restores elements' state when the model
 * requests viewer to refresh model structure. Since the viewer items are matched to the model elements using items'
 * indexes, inserting or removing elements in model can cause the expansion state of elements to shift after a refresh.
 * To compensate for this, the viewer saves the elements before a refresh is performed into a delta, but without
 * encoding elements using mementos. As the refresh of the tree progresses, the save state is restored to the tree and
 * elements are expanded or collapsed as needed to compensate for changes in model structure.
 * </p>
 * 
...
class ViewerStateTracker {
                    ...
                    if (!request.isCanceled() && (request.getStatus() == null || request.getStatus().isOK())) {
                        ...
                        try {
                            ...
                            if (stateDelta != null) {
                                ...
                                // Process start of restore in an async cycle because we may still be inside
                                // inputChanged()
     ...
     * 
     * @param rootDelta
     *        the {@link ModelDelta} to encode
     * @param defaultProvider
     *        the default provider to use when processing the given delta
     * 
     */
     ...
     * <p>
     * This method is called after every viewer update completion to continue restoring the expansion state that was
     * previously saved.
     * 
     * @param path
     *        the tree path to update
     * @param modelIndex
     *        the index in the current model
     * @param knowsHasChildren
     *        if the content provider knows it has children already
     * @param knowsChildCount
     *        if the content provider knows the current child count already
     * @param checkChildrenRealized
     *        if any realized children should be checked or not
     */
     ...
     * 
     * @param delta
     *        the {@link ModelDelta} to restore from
     * @param knowsHasChildren
     *        if the content provider has computed its children
     * @param knowsChildCount
     *        if the content provider has already computed the child count
     * @param checkChildrenRealized
     *        if any realized children should be checked
     */
     ...
     * scroll out as view content is filled in. <br>
     ...
     * 
     * @param path
     *        Path of element to cancel updates for.
     */
    ...
    void compareFinished(org.neuro4j.studio.debug.ui.views.ElementCompareRequest elementCompareRequest, ModelDelta delta) {
        notifyStateUpdate(elementCompareRequest.getViewerInput(), TreeModelContentProvider.UPDATE_COMPLETE, elementCompareRequest);
        if (DebugUIPlugin.DEBUG_STATE_SAVE_RESTORE && DebugUIPlugin.DEBUG_TEST_PRESENTATION_ID(fContentProvider.getPresentationContext())) {
            DebugUIPlugin.trace("\tSTATE END: " + elementCompareRequest + " = " + false); //$NON-NLS-1$ //$NON-NLS-2$
        }

        fCompareRequestsInProgress.remove(new CompareRequestKey(elementCompareRequest.getElementPath(), delta));
        if (!elementCompareRequest.isCanceled()) {
            if (elementCompareRequest.isEqual()) {
                delta.setElement(elementCompareRequest.getElement());
                restorePendingStateNode(delta, elementCompareRequest.knowsHasChildren(), elementCompareRequest.knowChildCount(), elementCompareRequest.checkChildrenRealized());
            } else if (elementCompareRequest.getModelIndex() != -1) {
                // Comparison failed.
                ...
                if ((delta.getFlags() & IModelDelta.REVEAL) != 0 && delta.getIndex() == elementCompareRequest.getModelIndex()) {
                    delta.setFlags(delta.getFlags() & ~IModelDelta.REVEAL);
                }
            }
        }
