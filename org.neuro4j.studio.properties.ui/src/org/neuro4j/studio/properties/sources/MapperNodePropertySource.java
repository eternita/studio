...
public class MapperNodePropertySource extends PropertySource {

    Map<Object, MapperListPropertySource> map = new HashMap<Object, MapperListPropertySource>();
    AdapterFactory af;

    public MapperNodePropertySource(Object object,
    ...
    {
        if (value instanceof MapperListPropertySource)
        {
            MapperNodeImpl mn = (MapperNodeImpl) object;
            MapperListPropertySource ps = (MapperListPropertySource) value;

            IItemPropertyDescriptor iItemPropertyDescriptor = itemPropertySource.getPropertyDescriptor(object, propertyId);
            iItemPropertyDescriptor.setPropertyValue(object, ps.getMapperNode().getKeyValue());
        } else {
            IItemPropertyDescriptor iItemPropertyDescriptor = itemPropertySource.getPropertyDescriptor(object, propertyId);
    ...
    public Object getPropertyValue(Object propertyId) {
        ...
        if ("KeyValue".equals(propertyId)) {
            MapperNodeImpl mapperNodeImpl = (MapperNodeImpl) object;
            if (map.containsKey(mapperNodeImpl.getId()))
            {
                return map.get(mapperNodeImpl.getId());
            } else {
                MapperListPropertySource desc = new MapperListPropertySource(mapperNodeImpl);
                map.put(mapperNodeImpl.getId(), desc);
                return desc;
            }
