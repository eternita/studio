...
public class FlowsListView extends ViewPart  {
	...
	private FlowViewLabelProvider fLabelProvider;
	...
	public void createPartControl(Composite parent) {
		...
		getSite().getWorkbenchWindow().addPerspectiveListener(new IPerspectiveListener2() {
			...
			public void perspectiveChanged(IWorkbenchPage page, IPerspectiveDescriptor perspective, IWorkbenchPartReference partRef, String changeId) {
				...
				if (part.equals(FlowsListView.this)) {
		...
		PatternFilter filter = new PatternFilter() {
			protected boolean isLeafMatch(Viewer viewer, Object element) {
				if (element instanceof FlowEntry) {
					FlowEntry logEntry = (FlowEntry) element;
...
//					String date = dateFormat.format(logEntry.getDate());
	...
	//	fFilteredTree.getViewer().setAutoExpandLevel(2);
		fFilteredTree.getViewer().setContentProvider(new FlowViewContentProvider(this));
...
//		});
	...
	private void createColumns(Tree tree) {
		...
		fColumn1.setText(Messages.FlowView_column_message);
...
//		});
	...
	void loadWorkflows() {
        ...
        List<FlowEntry> flows = FlowFromJarsLoader.getInstance().getAllFlows();
...
//        }
	...
	private String getTitleSummary() {
			return Messages.FlowView_WorkspaceLogFile;
	}
	...
	private void group(List entries) {
		if (true) {
			...
			for (Iterator i = entries.iterator(); i.hasNext();) {
				FlowEntry entry = (FlowEntry) i.next();
	...
	private void limitEntriesCount() {
		...
		Comparator dateComparator = new Comparator() {
			public int compare(Object o1, Object o2) {
				Date l1 = ((FlowEntry) o1).getDate();
				Date l2 = ((FlowEntry) o2).getDate();
				if ((l1 != null) && (l2 != null)) {
	 ...
	 */
	protected Group getGroup(FlowEntry entry) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				for (int i = 0; i < batchedEntries.size(); i++) {
					if (!monitor.isCanceled()) {
						FlowEntry entry = (FlowEntry) batchedEntries.get(i);
						pushEntry(entry);
	...
	private synchronized void pushEntry(FlowEntry entry) {
		if (true) {
...
//						IWorkbenchWindow window = Neuro4jCorePlugin.getDefault().getWorkbench().getActiveWorkbenchWindow();
//						if (window != null) {
...
//						break;
	...
	void onMouseHover(Event e) {
		...
		String message = null;
		if (item.getData() instanceof FlowEntry) {
			message = ((FlowEntry) item.getData()).getPluginId();
		} 
	...
	void onMouseMove(Event e) {
		...
		if (data instanceof FlowEntry) {
			FlowEntry entry = (FlowEntry) data;
		...
		if (i == DATE) {
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					long date1 = 0;
					long date2 = 0;
					if ((e1 instanceof FlowEntry) && (e2 instanceof FlowEntry)) {
						date1 = ((FlowEntry) e1).getDate().getTime();
						date2 = ((FlowEntry) e2).getDate().getTime();
			...
			fComparator = new Comparator() {
				public int compare(Object e1, Object e2) {
					if ((e1 instanceof FlowEntry) && (e2 instanceof FlowEntry)) {
						FlowEntry entry1 = (FlowEntry) e1;
						FlowEntry entry2 = (FlowEntry) e2;
	...
	private void readSettings() {
		...
		fMemento.putInteger(P_ORDER_TYPE, p.getInt(P_ORDER_TYPE, FlowsListView.DATE));
		fMemento.putBoolean(P_SHOW_FILTER_TEXT, p.getBoolean(P_SHOW_FILTER_TEXT, true));
		fMemento.putInteger(P_GROUP_BY, p.getInt(P_GROUP_BY, FlowsListView.GROUP_BY_NONE));
	}
